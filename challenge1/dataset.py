...
class ImgDataset(Dataset):
    ...
    def __getitem__(self,x):
        ...
        i = cv2.imread(f'D:/rover/ch1/data/{self.mode}/'+str(path)+'.jpg')
...
def getTrainDs(train_tr = None):
    train_df = pd.read_csv('D:/rover/ch1/data/train.csv')
    return ImgDataset(train_df,'train',train_tr)

def getValDs(val_tr):
    val_df = pd.read_csv('D:/rover/ch1/data/val.csv')
    return ImgDataset(val_df,'val',val_tr)

def getTestDs(test_tr):
    val_df = pd.read_csv('D:/rover/ch1/data/sample_submission.csv')
